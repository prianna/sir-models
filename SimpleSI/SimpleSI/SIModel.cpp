...
double beta=0.06; // Transmission term
double gamm=0.0005; // Mutation rate for WT virions
double mu=0.5; // Death rate & birth rate
double S0=5000; // Uninfected cells
double I0=500; // WT infected cells
double IM0=1; // Mutated infected cells

double MaxTime=60*365;
...
void Diff(double Pop[3])
{
  ...
  dPop[0] = beta*tmpS*tmpI - mu*tmpS;              // dS/dt
  dPop[1] = beta*tmpS*tmpI - gamm*(beta*tmpI*tmpS) - mu*tmpI;        // dI/dt
  dPop[2] = beta*tmpS*tmpIM + gamm*(beta*tmpI*tmpS) - mu*tmpIM;                        // dIM/dt
